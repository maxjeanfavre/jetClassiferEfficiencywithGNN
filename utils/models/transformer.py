...
class MultiHeadAttention(nn.Module):
    ...
    def get_dummy_scores(self, ip: torch.Tensor):
        """
        ### Calculate scores between queries and keys
        This method can be overridden for other variations like relative attention.
        """
        ...
        # Calculate $Q K^\top$ or $S_{ijbh} = \sum_d Q_{ibhd} K_{jbhd}$
        return torch.einsum('ibhd,jbhd->ijbh', ip, ip)
    ...
    def forward(self, *,
        ...
        ip = query
        head_shape = ip.shape[:-1]        
        ip = ip.view(*head_shape, self.heads, self.d_k) 
        ...
        dummy_scores = self.get_dummy_scores(ip)
        ...
        #print("ip.shape ",dummy_scores.shape)
        #print("scores ",scores.shape)
        #print("key ",key.shape)
        scores = torch.where(dummy_scores==0,1e-20,scores)
