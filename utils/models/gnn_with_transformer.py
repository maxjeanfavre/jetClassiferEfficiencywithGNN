...
from utils.models.transformer import MultiHeadAttention 
...
class GNN(Model):
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for x, _, target, one_event_idx in tqdm(dataloader_train, file=sys.stdout):
                ...
                if use_gpu:
                    ...
                    #print(target)
                    #print(torch.cat([x])
                    #print(type(one_event_idx))
                    #print(type(torch.tensor(np.array(one_event_idx))))
                    #print(type(target))
                    one_event_idx = one_event_idx.to(device)
                    x = x.to(device)
                    target = target.to(device)

                output = self.estimator(x,one_event_idx)
                ...
                if self.old_mode:
                    ...
                    print(f"training loss: {loss} for minibatch in epoch:{epoch}")
            ...
            with torch.no_grad():
                for x, _, target,one_event_idx in tqdm(dataloader_validation, file=sys.stdout):
                    if use_gpu:
                        x = x.to(device)
                        target = target.to(device)
                        one_event_idx = one_event_idx.to(device)
...
class TorchDataset(Dataset):
    def __init__(
        ...
        self.event_idx = torch.tensor(np.repeat(np.arange(self.n_events), self.event_n_jets), dtype=torch.int64)
        #print("setting self.event_idx ",self.event_idx)
        ...
        if self.preload:
            ...
            for i in tqdm(range(self.__len__()), file=sys.stdout):
                #print("inside tqdm ", i)
                g, flavour, target, one_event_idx = self.__getitem__single_event(idx=i)
                self.g_.append(g)
                self.flavour_.append(flavour)
                self.target_.append(target)
                self.one_event_idx_.append(one_event_idx)
            #print("final preloaded sets are :",len(self.g_)," ",len(self.one_event_idx_))    
            #print("final preloaded sets are :",len(self.g_))    
    ...
    def __getitem__(self, idx):
        if self.preload:
            ...
            #print("idx ",idx)
            #print("self.g_[idx] ",self.g_[idx])
            #print("self.flavour_[idx] ",self.flavour_[idx])
            #print("self.target_[idx] ",self.target_[idx])
            #print("self.one_event_idx_[idx] ",self.one_event_idx_[idx])

            g = self.g_[idx]
            flavour = self.flavour_[idx]
            target = self.target_[idx]
            one_event_idx = self.one_event_idx_[idx]
        else:
            g, flavour, target, one_event_idx = self.__getitem__single_event(idx=idx)

        return g, flavour, target, one_event_idx
    ...
    def __getitem__single_event(self, idx):
        ...
        one_event_idx = self.event_idx[r]
        #print("one_event_idx returning",one_event_idx)
...
def collate(samples):
    ...
    one_event_idx = [x[3] for x in samples]
    ...
    one_event_idx = torch.cat(one_event_idx)

    flavours = flavours.unsqueeze(1).float()
    targets = targets.long()
    one_event_idx = one_event_idx.long()
    ...
    return batched_graph, flavours, targets, one_event_idx
...
class GNNTorchModel(nn.Module):
    def __init__(
        ...
        self.mha = MultiHeadAttention(heads=8, d_model=512)
        self.mha.double()
        #print("self.mha is ",self.mha)
    ...
    def forward(self, g, one_event_idx):
        ...
        #print("one_event_idx ",one_event_idx)
        #print("one_event_idx ",len(one_event_idx))
        #print("batch size is ")
        gnn_final_out = g.ndata[self.node_hidden_state_key]
        #print("gnn_final_out ",gnn_final_out.is_cuda)
        #print("one_event_idx ",one_event_idx.is_cuda)
        #from collections import Counter
        #print(one_event_idx)
        MyList=list(one_event_idx.cpu().numpy())
        event_n_jets = list({i:MyList.count(i) for i in MyList}.values())
        event_n_jets_offset = np.insert(np.cumsum(event_n_jets), 0, 0, axis=0)[:-1] 
        #event_n_jets_offset = np.cumsum(event_n_jets)

        from itertools import islice
        it = iter(gnn_final_out)
        sliced_gnn_output =[list(islice(it, 0, i)) for i in event_n_jets]

        #max_jets = max([t.size(dim=0) for t in one_event_idx]) 
        max_jets = max(event_n_jets)
        #padded_jets = [torch.nn.functional.pad(t, [0, max_jets - t.size(dim=0)],value=-1) for  t in ip]
        #n=3
        #print("one_event_idx now ",one_event_idx.is_cuda) 
        transformer_ip = torch.tensor(np.ones((max_jets,len(event_n_jets),512))*0)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu") 
        transformer_ip.to(device)
        #print("transformer_ip ",transformer_ip.is_cuda)
        #print(transformer_ip )
        #print("gnn_final_out ",gnn_final_out)
        #test = torch.tensor(np.array([1,2,3]))
        #test.to(device)
        #print("test cuda ",test.is_cuda)
        for i in range(max_jets):
            for j in range(len(event_n_jets)):
                if event_n_jets[j] > i:
                    transformer_ip[i][j] = sliced_gnn_output[j][i]
                    #print(sliced_gnn_output[j][i].shape)
                #else:
                    #print(event_n_jets[j], "i ", i, " ")
                    #transformer_ip[i][j] = torch.tensor([-1]*512

        #print(transformer_ip[:10])   
        #print(sliced_gnn_output)
        #print(transformer_ip.shape)
        #print(gnn_final_out.shape)
        #print(self.mha)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu") 
        transformer_ip = transformer_ip.double().to(device)
        #self.mha.to(device)
        #print("target.cuda ",g.is_cuda)
        #print("transformer_ip now ",transformer_ip.is_cuda) 
        mha = self.mha(query=transformer_ip,key=transformer_ip,value=transformer_ip)
        #print(mha.shape)

        transformer_out = torch.zeros((sum(event_n_jets),512))
        transformer_out = transformer_out.float().to(device)
        #for i in range(max_jets):
        for j in range(len(event_n_jets)):
            i=0
            while event_n_jets[j] > i:
                    #print(i,j,transformer_ip[i][j])
                    transformer_out[event_n_jets_offset[j]+i] = mha[i][j]
                    i +=1
        #print("transformer_out ",transformer_out.shape)
        #print("transformer_ip ",transformer_ip.shape)
        #print("event_n_jets ",event_n_jets.shape)
        ...
        #    dim=1,
        #)
        #print(g.ndata[self.node_features_key].is_cuda)
        #print(transformer_out.is_cuda)
        jet_efficiency_net_input = torch.cat(
            [
                g.ndata[self.node_features_key],
                transformer_out,
        ...
        #print("jet_efficiency_net_input.shape ",jet_efficiency_net_input.shape)
