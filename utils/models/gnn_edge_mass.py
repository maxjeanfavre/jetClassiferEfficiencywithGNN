# GNN with single mass edge feature
...
class GNN(Model):
    ...
    def train(
        ...
        logger.debug(
            "Jet_mass_train type " f"{Jet_mass_train}" " and " f"{type(Jet_mass_train)}"
            " and " f"{Jet_mass_train.ndim}"
        )
    ...
    def get_required_columns(self) -> Tuple[str, ...]:
        ...
        required_columns.extend(["Jet_mass"])
...
def get_edge_dict_Jet_mass(max_n):
    edge_dict = {}
...
class TorchDataset(Dataset):
    def __init__(
        ...
        self.edge_dict = get_edge_dict_Jet_mass(max_n=np.max(self.event_n_jets)) #for Jet_mass as a edge feature
    ...
    def __getitem__single_event(self, idx):
        ...
        Jet_mass = self.Jet_mass_tensor[r]
        ...
        g.ndata[NODE_FEATURES_KEY] = node_features
        g.ndata[FLAVOUR_INDICES_KEY] = flavour_indices
        g.edata[EDGE_FEATURE_KEY] = Jet_mass
        #g.edata[EDGE_FEATURE_KEY] = delta_r
        #print("src ",src)
        #print("dst ",dst)
        #print("g ",g)
