...
from utils.models.attention import GraphAttentionLayer
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for x, _, target in tqdm(dataloader_train, file=sys.stdout):
                ...
                grad_sum = 0
                #print(self.estimator.parameters())
...
class GNNTorchModel(nn.Module):
    def __init__(
        ...
        #dropout = 0.3
        #self.gat = GraphAttentionLayer(in_features = 512, out_features = 512, n_heads= 1, dropout = dropout)    
        #logger.trace(
        #    "drop out is set to "
        #    f"{dropout}"
        #)
        #dropout = 0.2
        #n_heads = 8
        self.gat = GraphAttentionLayer(in_features = 512, out_features = 512, n_heads= self.attention_network_n_heads, dropout = self.attention_network_dropout, is_concat = self.attention_network_is_concat)    
        #logger.trace(
        #    "drop out is set to "
...
#        print("g.ndata[self.node_hidden_state_key] before jet layers ",g.ndata[self.node_hidden_state_key])
        ...
        #print("g.adj(etype='develops') ",g.adj())  
        #print("type(g.adj() )",type(g.adj()))
        nodes = g.adj().shape[0] 
        ...
        #print("gat_output.shape ",gat_output.shape)
