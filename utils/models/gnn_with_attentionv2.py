...
class GNN(Model):
    ...
    def __init__(
        ...
        old_mode: bool = False,
        attention_network_is_concat: bool=True,
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            for _, (x, f, t) in enumerate(tqdm(dataloader, file=sys.stdout)):
                if use_gpu:
                    x = x.to(device)
                    print(x,f,t)
                    print("g.ndata[FLAVOUR_FEATURES_KEY] ",x.ndata["FLAVOUR_INDICES"])
                    print("g.ndata[NODE_FEATURES_KEY] ",x.ndata[NODE_FEATURES_KEY])
                    print("g.ndata[NODE_FEATURES_KEY].shape ",x.ndata[NODE_FEATURES_KEY].shape)
                    print("g.edata[EDGE_FEATURE_KEY] ",x.edata[EDGE_FEATURE_KEY])
...
class GNNTorchModel(nn.Module):
    def __init__(
        ...
        attention_network_dropout: float,
        attention_network_n_heads: int,
        ...
        #dropout = 0.1
        #n_heads = 8
        #print("attention_network_n_heads ",type(self.attention_network_n_heads))
