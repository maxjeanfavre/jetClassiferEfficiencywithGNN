...
from utils.helpers.accuracy import my_metrics
from utils.models.model import Model
from utils.models.attentionv2 import GraphAttentionV2Layer
...
class GNN(Model):
    ...
    def __init__(
        ...
        attention_network_dropout: int,
        attention_network_n_heads: int,
        old_mode: bool = False,
        attention_network_is_concat: bool,
        ...
        self.attention_network_dropout = attention_network_dropout
        self.attention_network_n_heads = attention_network_n_heads
        self.attention_network_is_concat = attention_network_is_concat
        ...
        self.estimator = GNNTorchModel(
            ...
            attention_network_dropout = self.attention_network_dropout,
            attention_network_n_heads = self.attention_network_n_heads,
            attention_network_is_concat = self.attention_network_is_concat,
        )
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for x, _, target in tqdm(dataloader_train, file=sys.stdout):
                ...
                #epoch_train_accuracy += my_metrics.accuracy(output, target)
                
                print(f"training loss: {loss} for minibatch in epoch:{epoch}")
                #print("total training events in class 0: ",(target==0).sum()," class 1: ",(target==1).sum()," class 2: ",(target==2).sum()," class 3: ",(target==3).sum() )
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            #epoch_train_accuracy /= len(dataloader_train)
            ...
            with torch.no_grad():
                for x, _, target in tqdm(dataloader_validation, file=sys.stdout):
                    ...
                    #epoch_validation_accuracy += my_metrics.accuracy(output, target)
                    
                    #print(f"validation loss: {loss} and accuracy: {epoch_validation_accuracy}")
            ...
            logger.info(
               ...
               # f"epoch train accuracy: {epoch_train_accuracy},"
                f"epoch validation loss: {epoch_validation_loss}, "
               # f"epoch validation accuracy: {epoch_validation_accuracy},"
...
class GNNTorchModel(nn.Module):
    def __init__(
        ...
        #n_heads = 8
        #is_concat = False
        self.gat = GraphAttentionV2Layer(in_features = 512, out_features = 512, n_heads= self.attention_network_n_heads, dropout = self.attention_network_dropout, is_concat = self.attention_network_is_concat)    
        logger.trace(
            "drop out is set to "
            f"{dropout} and nheads is set to "
            f"{n_heads}"
            f" with {is_concat} set for is_concat in attention"
        )
    ...
    def forward(self, g):
        ...
        nodes = g.adj().shape[0] 
        #print("g.adj(etype='dR').shape ",g.adj().to_dense().reshape((nodes,nodes,1)))
        #adj_matrix = g.adj()[:,None]
        adj_mat = g.adj().to_dense().reshape((nodes,nodes,1))
        #print("adj_mat.shape ",adj_mat.shape)
        #print("adj_mat type ",type(adj_mat))
        #print("adj_mat cuda ",adj_mat.is_cuda)
        ...
        for edge_update, node_update in zip(self.edge_updates, self.node_updates):
            ...
            g.ndata[self.node_hidden_state_key] = self.gat(h= g.ndata[self.node_hidden_state_key], adj_mat= adj_mat)
...
#        print("g.ndata[self.node_hidden_state_key] before jet layers ",g.ndata[self.node_hidden_state_key])
        ...
        #print("adj_mat cuda ",adj_mat.is_cuda)
        #print("adj mat  ")
        #gat_output = self.gat(h= g.ndata[self.node_hidden_state_key], adj_mat= adj_mat)
