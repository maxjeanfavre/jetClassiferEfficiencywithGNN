import copy

from configs.model.gnn_edge_dr_mass_eucl_norm import model_config as gnn_edge_dr_massdijet_model_config
from utils.preprocessing.identity import Identity
from utils.preprocessing.normalizer import Normalizer

model_config = copy.deepcopy(gnn_edge_dr_massdijet_model_config)

model_config.name = "gnn_node_njet_edge_mass_eucl_norm_dr"

model_config.model_init_kwargs["node_features_cols"].extend(
    [
        "nJet",
    ]
)

model_config.model_init_kwargs["preprocessing_pipeline"].column_preprocessors.update(
    {
        "nJet": Normalizer(),
    }
)